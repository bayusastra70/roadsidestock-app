...
export default async function HomePage({
  ...
  if (!warungId) redirect("/login");

  // AMBIL DATA WARUNG & USER UNTUK SAPAAN
  const currentWarung = await prisma.warung.findUnique({
    where: { id: warungId },
    include: { user: true }
  });
  ...
  // DATA RINGKASAN
  const allProducts = await prisma.product.findMany({ where: { warungId } });
    ...
    <div className="min-h-screen bg-gray-50 pb-40 font-sans text-black">
      ...
      <div className="p-6 bg-white border-b border-gray-100 sticky top-0 z-10 flex flex-col gap-4 shadow-sm">
        <div className="flex justify-between items-center">
          <div>
            <p className="text-blue-600 text-[10px] font-black uppercase tracking-widest">Halo, {currentWarung?.user.nama} 👋</p>
            <h1 className="text-xl font-black text-gray-800 italic">{currentWarung?.nama}</h1>
          </div>
          {/* TOMBOL TAMBAH DENGAN TULISAN AGAR JELAS */}
          <Link href="/tambah" className="bg-blue-600 text-white px-4 py-2 rounded-full flex items-center gap-2 shadow-lg shadow-blue-100 active:scale-95 transition-all">
            ...
            <span className="text-xs font-black uppercase">Barang</span>
          </Link>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid gap-4">
          <h2 className="font-bold text-gray-700 text-sm flex items-center gap-2">
            ...
            Stok {currentCat !== "Semua" ? currentCat : "Barang"}
          </h2>
          ...
          {products.length === 0 ? (
            ...
            products.map((product) => {
              ...
              return (
                <div key={product.id} className={`bg-white p-5 rounded-3xl shadow-sm border flex flex-col gap-4 transition-all ${isLowStock ? 'border-orange-200 bg-orange-50/20' : 'border-gray-100'}`}>
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      <Link href={`/edit/${product.id}`} className="p-2 bg-blue-50 text-blue-600 rounded-xl active:scale-90">
...
<form 
  action={jualAksi} 
  className="flex bg-gray-50 rounded-2xl p-1 border border-gray-100 items-center"
>
  ...
  <button 
    ...
    className={`px-4 py-2 rounded-xl text-[10px] font-black active:scale-95 transition-all ${
      product.stock <= 0 ? 'bg-gray-300 text-white' : 'bg-blue-600 text-white shadow-md shadow-blue-100'
    }`}
  >
